...
async function runGitHub(step, context, interpolate) {
    ...
    // Support both "add_comment" (UI label) and "comment_issue" (legacy key)
    if (action === "add_comment" || action === "comment_issue") {
            ...
            if (!owner || !repo || !number || !comment) throw new Error("add_comment requires owner, repo, issue_number, and comment");
