...
const mockVersionModel = {
  create: async (data) => {
    ...
    const exists = mockVersionStore.some(v => 
      String(v.workflowId) === String(data.workflowId) && 
...
async function runTests() {
  ...
  try {
    ...
    const workflow = await mockWorkflowModel.create({
      ...
      metadata: {
        steps: [
          { stepId: "step-1", type: "llm", prompt: "Hello world" }
        ],
        edges: []
      }
    });
    ...
    const v1 = await workflowVersionService.createVersionIfNeeded(workflow, userId, "Initial version");
    
    if (!v1 || v1.versionNumber !== 1) {
      console.error(`❌ Test 1 Failed: Expected versionNumber 1, got ${v1 ? v1.versionNumber : "null"}`);
      passed = false;
    } else {
    ...
    const vSame = await workflowVersionService.createVersionIfNeeded(workflow, userId, "Identical save");
    
    if (!vSame || vSame.versionNumber !== 1) {
      console.error(`❌ Test 2 Failed: Duplicate save created a new version: ${vSame ? vSame.versionNumber : "null"}`);
      passed = false;
    } else {
    ...
    const v2 = await workflowVersionService.createVersionIfNeeded(workflow, userId, "Updated details");
    if (!v2 || v2.versionNumber !== 2) {
      console.error(`❌ Test 3 Failed: Expected versionNumber 2, got ${v2 ? v2.versionNumber : "null"}`);
      passed = false;
    } else if (v2.workflowSnapshot.description !== "Updated description") {
      console.error(`❌ Test 3 Failed: Snapshot description not updated in v2: ${v2.workflowSnapshot.description}`);
      passed = false;
    } else {
    ...
    const v3 = await workflowVersionService.createVersionIfNeeded(workflow, userId, "Added delay step");
    if (!v3 || v3.versionNumber !== 3) {
      console.error(`❌ Test 4 Failed: Expected versionNumber 3, got ${v3 ? v3.versionNumber : "null"}`);
      passed = false;
    } else if (v3.workflowSnapshot.metadata.steps.length !== 2) {
      console.error(`❌ Test 4 Failed: Snapshot steps count not correct in v3: ${v3.workflowSnapshot.metadata.steps.length}`);
      passed = false;
    } else {
    ...
    for (let i = 1; i <= 3; i++) {
      concurrencyPromises.push((async (index) => {
        ...
        return await workflowVersionService.createVersionIfNeeded(tempWorkflow, userId, `Concurrent update ${index}`);
      })(i));
    }
    ...
    if (!match) {
      console.error(`❌ Test 5 Failed: Expected sequential version numbers [4, 5, 6], got:`, versionNumbers);
      passed = false;
    } else {
      console.log("✅ Test 5 Passed: Concurrent requests successfully ordered sequentially via retry loop.");
    }
    ...
    const rolledWorkflow = await workflowVersionService.rollback(preRollbackDbState, targetVersion._id, userId);
    
    if (rolledWorkflow.description !== "Updated description") {
      console.error(`❌ Test 6 Failed: Reverted description mismatch: expected 'Updated description', got '${rolledWorkflow.description}'`);
      passed = false;
    } else if (rolledWorkflow.metadata.steps.length !== 1) {
      console.error(`❌ Test 6 Failed: Reverted steps mismatch: expected 1 step, got ${rolledWorkflow.metadata.steps.length}`);
      passed = false;
    } else {
    ...
    const postRollbackLatest = [...mockVersionStore].sort((a, b) => b.versionNumber - a.versionNumber)[0];
    if (postRollbackLatest.versionNumber !== 7) {
      console.error(`❌ Test 6 verification Failed: Expected post-rollback latest version 7, got ${postRollbackLatest.versionNumber}`);
      passed = false;
    } else if (!postRollbackLatest.note.includes("Pre-rollback to v2")) {
      console.error(`❌ Test 6 verification Failed: Expected pre-rollback version note, got '${postRollbackLatest.note}'`);
      passed = false;
    } else {
      console.log("✅ Test 6 Verification Passed: Reversibility verified. Pre-rollback state captured as v7.");
    }
