...
function getStepDescription(step: BackendStep) {
  ...
  if (type === "document_query") return step.query ? `Query: ${step.query.slice(0, 100)}` : "Document Query";
      ...
      <DialogContent className="max-w-4xl h-[80vh] flex flex-col p-0 overflow-hidden bg-background border border-border shadow-2xl rounded-xl">
        <DialogHeader className="px-6 py-4 border-b border-border bg-muted/20">
          <div className="flex items-center gap-2">
            ...
            <div>
              <DialogTitle className="text-xl font-bold tracking-tight">Workflow Version History</DialogTitle>
        ...
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-sm text-muted-foreground animate-pulse">Loading version history...</p>
          </div>
        ) : error ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 p-6 text-center">
            ...
            <Button variant="outline" size="sm" onClick={fetchVersions}>Try Again</Button>
          </div>
        ) : versions.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-2 text-center p-6">
            ...
            <p className="text-sm text-muted-foreground max-w-xs">
              Make config changes to steps or properties to generate version snapshots automatically.
            </p>
          </div>
        ) : (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <Badge variant="outline" className={`font-mono text-xs ${
                            isActive ? "border-primary/40 bg-primary/20 text-primary" : "bg-muted text-muted-foreground"
                          }`}>
                ...
                <div className="flex-1 flex flex-col min-h-0 relative">
                  ...
                  <div className="p-6 border-b border-border bg-muted/5">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="outline" className="border-primary/20 bg-primary/5 text-primary font-mono font-semibold px-2 py-0.5">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2 items-center text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center gap-1 bg-muted px-2.5 py-1 rounded-full border border-border">
                        ...
                        <span>Agent ID: {selectedVersion.workflowSnapshot.agentId ? selectedVersion.workflowSnapshot.agentId.slice(-6).toUpperCase() : "None Assigned"}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <ScrollArea className="flex-1 p-6">
                    <h4 className="text-sm font-semibold text-muted-foreground mb-4 uppercase tracking-wider">
                      Steps Pipeline ({selectedVersion.workflowSnapshot.metadata?.steps?.length || 0})
                    </h4>
                    ...
                    <div className="space-y-3">
                      ...
                      selectedVersion.workflowSnapshot.metadata.steps.length > 0 ? (
                        selectedVersion.workflowSnapshot.metadata.steps.map((step, index) => {
                          const stepsCount = selectedVersion.workflowSnapshot.metadata?.steps?.length || 0;
                              ...
                              <div className="flex items-start gap-3 p-3.5 rounded-lg border border-border bg-card">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <Badge variant="outline" className="text-[10px] font-medium py-0 px-1.5 uppercase font-mono bg-muted/50">
                    ...
                    {!showConfirm ? (
                      ...
                      <div className="w-full flex flex-col gap-3 p-3 rounded-lg border border-warning/30 bg-warning/5 animate-in fade-in slide-in-from-bottom-2">
                        <div className="flex items-start gap-2.5">
                          ...
                          <div>
                            <h5 className="font-semibold text-sm text-foreground">Confirm Rollback</h5>
                            <p className="text-xs text-muted-foreground mt-0.5">
                              Are you sure you want to revert the workflow pipeline to version <strong>v{selectedVersion.versionNumber}</strong>? 
                              Your current layout will be saved as a new version, so this operation remains fully reversible.
                            </p>
                          </div>
                        </div>
